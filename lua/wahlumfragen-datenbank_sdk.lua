...
require("wahlumfragen-datenbank_types")
...
local WahlumfragenDatenbankSDK = {}
WahlumfragenDatenbankSDK.__index = WahlumfragenDatenbankSDK
...
WahlumfragenDatenbankSDK._make_feature = _make_feature


function WahlumfragenDatenbankSDK.new(options)
  local self = setmetatable({}, WahlumfragenDatenbankSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WahlumfragenDatenbankSDK:options_map()
...
function WahlumfragenDatenbankSDK:get_utility()
  return Utility.copy(self._utility)
end


function WahlumfragenDatenbankSDK:get_root_ctx()
  return self._rootctx
end


function WahlumfragenDatenbankSDK:prepare(fetchargs)
...
function WahlumfragenDatenbankSDK:direct(fetchargs)
...
function WahlumfragenDatenbankSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function WahlumfragenDatenbankSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "WahlumfragenDatenbankSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function WahlumfragenDatenbankSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WahlumfragenDatenbankSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WahlumfragenDatenbankSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WahlumfragenDatenbankSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WahlumfragenDatenbankSDK.new(sdkopts)
  sdk.mode = "test"
...
return WahlumfragenDatenbankSDK
