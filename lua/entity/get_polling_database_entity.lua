...
local GetPollingDatabaseEntity = {}
GetPollingDatabaseEntity.__index = GetPollingDatabaseEntity


function GetPollingDatabaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetPollingDatabaseEntity)
  self._name = "get_polling_database"
  self._client = client
...
function GetPollingDatabaseEntity:get_name()
  return self._name
end


function GetPollingDatabaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetPollingDatabaseEntity.new(self._client, opts)
end
...
function GetPollingDatabaseEntity:mark_deleted()
  self._deleted = true
end


function GetPollingDatabaseEntity:deleted()
  return true == self._deleted
end


function GetPollingDatabaseEntity:data_set(args)
  if args ~= nil then
...
function GetPollingDatabaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetPollingDatabaseEntity:match_set(args)
  if args ~= nil then
...
function GetPollingDatabaseEntity:match_get()
...
function GetPollingDatabaseEntity:stream(action, args, callopts)
...
function GetPollingDatabaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return GetPollingDatabaseEntity
