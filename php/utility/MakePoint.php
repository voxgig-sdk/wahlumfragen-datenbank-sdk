...
// WahlumfragenDatenbank SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class WahlumfragenDatenbankMakePoint
{
    public static function call(WahlumfragenDatenbankContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof WahlumfragenDatenbankError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = WahlumfragenDatenbankHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = WahlumfragenDatenbankHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
