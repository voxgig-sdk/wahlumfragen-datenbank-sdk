...
// WahlumfragenDatenbank SDK feature test
//
...
class FtCtrl extends WahlumfragenDatenbankControl
{
...
class FtClient
{
    ...
    private WahlumfragenDatenbankUtility $utility;

    public function __construct(WahlumfragenDatenbankUtility $utility, array $options)
    {
    ...
    public function get_utility(): WahlumfragenDatenbankUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WahlumfragenDatenbankContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WahlumfragenDatenbankContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WahlumfragenDatenbankContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WahlumfragenDatenbankConfig::make_feature($name);
    ...
    private static function buildUrl(WahlumfragenDatenbankSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WahlumfragenDatenbankError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WahlumfragenDatenbankResponse($fetched);
        }
    ...
    private function fail(WahlumfragenDatenbankContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WahlumfragenDatenbankContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
