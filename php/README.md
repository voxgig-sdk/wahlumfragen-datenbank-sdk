# WahlumfragenDatenbank PHP SDK



The PHP SDK for the WahlumfragenDatenbank API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->GetPollingDatabase()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/wahlumfragen-datenbank-sdk/releases](https://github.com/voxgig-sdk/wahlumfragen-datenbank-sdk/releases)
...
try {
    ...
    foreach ($getpollingdatabases as $record) {
...
print_r(array_map(fn($item) => $item->data_get(), $getpollingdatabase));
```
...
```
WAHLUMFRAGEN_DATENBANK_TEST_LIVE=TRUE
```
...
```php
$client = WahlumfragenDatenbankSDK::test($testopts, $sdkopts);
```

Creates a test-mode client with mock transport. Both arguments may be `null`.

### WahlumfragenDatenbankSDK methods
...
| `GetPollingDatabase` | `($data): GetPollingDatabaseEntity` | Create a GetPollingDatabase entity instance. |
...
```
php/
├── wahlumfragendatenbank_sdk.php          -- Main SDK class
...
The main class (`wahlumfragendatenbank_sdk.php`) exports the SDK class
...
$getpollingdatabase->list();

// $getpollingdatabase->data_get() now returns the getpollingdatabase data from the last list
// $getpollingdatabase->match_get() returns the last match criteria
```
