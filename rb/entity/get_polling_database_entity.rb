...
class GetPollingDatabaseEntity
  ...
  def make
    opts = @_entopts.dup
    GetPollingDatabaseEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WahlumfragenDatenbankHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WahlumfragenDatenbankHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [GetPollingDatabaseListMatch, Hash, nil] match filter (any subset of
  #   GetPollingDatabase fields); defaults to nil, treated as an empty match that lists all.
