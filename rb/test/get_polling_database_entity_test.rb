...
class GetPollingDatabaseEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = WahlumfragenDatenbankSDK.test(seed, nil)
    seen = base.GetPollingDatabase(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.GetPollingDatabase(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WAHLUMFRAGEN_DATENBANK_TEST_GET_POLLING_DATABASE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def get_polling_database_basic_setup(extra)
  ...
  entid_env_raw = ENV["WAHLUMFRAGEN_DATENBANK_TEST_GET_POLLING_DATABASE_ENTID"]
