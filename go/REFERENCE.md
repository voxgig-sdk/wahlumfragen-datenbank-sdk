# WahlumfragenDatenbank Golang SDK Reference

Complete API reference for the WahlumfragenDatenbank Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *WahlumfragenDatenbankSDK`
...
#### `GetPollingDatabase(data map[string]any) WahlumfragenDatenbankEntity`

Create a new `GetPollingDatabase` entity instance. Pass `nil` for no initial data.

#### `Metadata(data map[string]any) WahlumfragenDatenbankEntity`
...
| `Results` | `map[string]any` | Yes | Polling results indexed by party ID |
