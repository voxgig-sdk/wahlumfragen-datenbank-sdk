...
import (
	...
	omni "github.com/voxgig-sdk/wahlumfragen-datenbank-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WahlumfragenDatenbankError); is {
				out := map[string]any{
					"name":    "WahlumfragenDatenbankError",
...
func omniCtx(arg any, client *sdk.WahlumfragenDatenbankSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WahlumfragenDatenbankSDK); is {
			client = live
