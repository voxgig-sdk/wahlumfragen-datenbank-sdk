...
func TestGetPollingDatabaseEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetPollingDatabase(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetPollingDatabase(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set WAHLUMFRAGEN_DATENBANK_TEST_GET_POLLING_DATABASE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		getPollingDatabaseRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_polling_database")))
...
func get_polling_databaseBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WAHLUMFRAGEN_DATENBANK_TEST_GET_POLLING_DATABASE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WAHLUMFRAGEN_DATENBANK_TEST_GET_POLLING_DATABASE_ENTID"])
