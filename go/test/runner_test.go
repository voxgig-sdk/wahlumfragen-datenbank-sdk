...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("WAHLUMFRAGENDATENBANK_TEST_LIVE") == "TRUE" ||
		os.Getenv("WAHLUMFRAGENDATENBANK_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("WAHLUMFRAGENDATENBANK_TEST_EXPLAIN"); explain != "" {
