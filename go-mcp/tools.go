...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_polling_database | metadata"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.WahlumfragenDatenbankSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "wahlumfragen-datenbank_list",
		Description: "List records from WahlumfragenDatenbank. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "wahlumfragen-datenbank_load",
		Description: "Load a single record from WahlumfragenDatenbank. " +
...
func runOp(client *sdk.WahlumfragenDatenbankSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.WahlumfragenDatenbankSDK, name string) (sdk.WahlumfragenDatenbankEntity, error) {
	switch strings.ToLower(name) {
	case "get_polling_database":
		return client.GetPollingDatabase(nil), nil
	case "metadata":
