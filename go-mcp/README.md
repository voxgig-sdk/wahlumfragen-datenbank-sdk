# wahlumfragen-datenbank-mcp

[MCP](https://modelcontextprotocol.io) server exposing the WahlumfragenDatenbank SDK as
two agent tools — `wahlumfragen-datenbank_list` and `wahlumfragen-datenbank_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/wahlumfragen-datenbank-mcp)
make build

# 2. Provide credentials via the environment
export WAHLUMFRAGEN_DATENBANK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user wahlumfragen-datenbank \
  -- /absolute/path/to/wahlumfragen-datenbank-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./wahlumfragen-datenbank-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// wahlumfragen-datenbank_list: first page of records
{ "entity": "get_polling_database" }
{ "entity": "get_polling_database", "query": { } }

// wahlumfragen-datenbank_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/wahlumfragen-datenbank-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user wahlumfragen-datenbank \
     -- "$PWD"/dist/*/wahlumfragen-datenbank-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `wahlumfragen-datenbank_list` and `wahlumfragen-datenbank_load` tools now appear
   in new sessions. Ask the agent to *"list get_polling_database using wahlumfragen-datenbank"*
   and it calls `wahlumfragen-datenbank_list` with `{"entity":"get_polling_database"}`.
...
```sh
export WAHLUMFRAGEN_DATENBANK_APIKEY=sk_live_xxx            # API key
export WAHLUMFRAGEN_DATENBANK_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `wahlumfragen-datenbank_list` tool
...
### Call the `wahlumfragen-datenbank_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `wahlumfragen-datenbank_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `wahlumfragen-datenbank_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WAHLUMFRAGEN_DATENBANK_APIKEY` | API key sent with every request. |
| `WAHLUMFRAGEN_DATENBANK_BASE` | Optional override of the API base URL. |
...
```sh
./wahlumfragen-datenbank-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"wahlumfragen-datenbank_load","arguments":{"entity":"metadata","query":{"id":1}}}}'
```
