...
class TestGetPollingDatabaseDirect:

    def test_should_direct_list_get_polling_database(self):
        setup = _get_polling_database_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-get_polling_database", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_polling_database_direct_setup(mockres):
