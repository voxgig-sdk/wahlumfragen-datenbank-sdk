# WahlumfragenDatenbank SDK test runner
...
class WahlumfragenDatenbankTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WahlumfragenDatenbankTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WahlumfragenDatenbankTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WahlumfragenDatenbankTestRunner.getenv("WAHLUMFRAGEN_DATENBANK_TEST_LIVE")
        override = WahlumfragenDatenbankTestRunner.getenv("WAHLUMFRAGEN_DATENBANK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WahlumfragenDatenbankTestRunner.getenv(key)
        ...
        explain = WahlumfragenDatenbankTestRunner.getenv("WAHLUMFRAGEN_DATENBANK_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["WAHLUMFRAGEN_DATENBANK_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if WahlumfragenDatenbankTestRunner._test_control is not None:
            return WahlumfragenDatenbankTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WahlumfragenDatenbankTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WahlumfragenDatenbankTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WahlumfragenDatenbankTestRunner.load_test_control()
...
def load_env_local():
    WahlumfragenDatenbankTestRunner.load_env_local()


def env_override(m):
    return WahlumfragenDatenbankTestRunner.env_override(m)


def entity_data(v):
    return WahlumfragenDatenbankTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WahlumfragenDatenbankTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WahlumfragenDatenbankTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WahlumfragenDatenbankTestRunner.load_test_control()


def live_client_options():
    return WahlumfragenDatenbankTestRunner.live_client_options()


def live_delay_ms():
    return WahlumfragenDatenbankTestRunner.live_delay_ms()
