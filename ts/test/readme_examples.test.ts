...
//     (\`new WahlumfragenDatenbankSDK(...)\` / \`WahlumfragenDatenbankSDK.test(...)\` become
//     \`WahlumfragenDatenbankSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WahlumfragenDatenbankSDK'
...
const TEST_SEED = {"entity":{"get_polling_database":{"test01":{"id":"test01"}},"metadata":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WahlumfragenDatenbankSDK(...)\` and any
// \`WahlumfragenDatenbankSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WahlumfragenDatenbankSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WahlumfragenDatenbankSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
