...
boru-driven command-line client **and** interactive REPL for the WahlumfragenDatenbank
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/wahlumfragen-datenbank-cli)
make build

# 2. See usage (words, entities, env vars)
./wahlumfragen-datenbank-cli --help
...
./wahlumfragen-datenbank-cli list metadata

# 5. Override the API base URL for a single call
WAHLUMFRAGEN_DATENBANK_BASE=https://api.example.com ./wahlumfragen-datenbank-cli list get_polling_database

# 6. No arguments -> interactive REPL
./wahlumfragen-datenbank-cli
wahlumfragen-datenbank> list get_polling_database
wahlumfragen-datenbank> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/wahlumfragen-datenbank-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/wahlumfragen-datenbank-cli list get_polling_database
   ```
...
Run with no arguments to open a REPL (prompt `wahlumfragen-datenbank>`). Each line is
evaluated as its own boru expression:

```text
$ ./wahlumfragen-datenbank-cli
wahlumfragen-datenbank> list get_polling_database
wahlumfragen-datenbank> /help
wahlumfragen-datenbank> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_polling_database`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/wahlumfragen-datenbank-cli`. |
...
(`./wahlumfragen-datenbank-cli <expr>`) and interactively (the REPL), and expressions compose the
