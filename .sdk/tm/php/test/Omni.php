...
//    only - a typed WahlumfragenDatenbankContext there would make every ctx
...
final class WahlumfragenDatenbankOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new WahlumfragenDatenbankContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new WahlumfragenDatenbankSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new WahlumfragenDatenbankResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new WahlumfragenDatenbankError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new WahlumfragenDatenbankResponse($clean['response']);
    ...
    public static function err_from_map($m): ?WahlumfragenDatenbankError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new WahlumfragenDatenbankError(is_string($code) ? $code : '', $msg);
    }
