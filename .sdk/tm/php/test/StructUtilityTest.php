...
// WahlumfragenDatenbank SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = WahlumfragenDatenbankOmni::makeStructRunner(
    ...
    {
        return WahlumfragenDatenbankOmni::objmodel(WahlumfragenDatenbankOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return WahlumfragenDatenbankOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return WahlumfragenDatenbankOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(WahlumfragenDatenbankOmni::structfix($now, Struct::undef()))
                        !== json_encode(WahlumfragenDatenbankOmni::structfix($val, Struct::undef()))) {
