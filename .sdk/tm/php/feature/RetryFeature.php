...
// WahlumfragenDatenbank SDK retry feature
...
class WahlumfragenDatenbankRetryFeature extends WahlumfragenDatenbankBaseFeature
{
    ...
    public function init(WahlumfragenDatenbankContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WahlumfragenDatenbankContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WahlumfragenDatenbankContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WahlumfragenDatenbankContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
