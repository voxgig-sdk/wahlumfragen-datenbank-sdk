...
module WahlumfragenDatenbankUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WahlumfragenDatenbankResponse.new({})
    ...
    if fetch_err
      ...
      response = WahlumfragenDatenbankResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WahlumfragenDatenbankResponse.new(fetched)
    else
